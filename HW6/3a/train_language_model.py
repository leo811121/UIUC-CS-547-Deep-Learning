...
vocab_size = 8000
x_train = []
with io.open('/u/training/tra335/HW5/1a/preprocessed_data/imdb_train.txt','r',encoding='utf-8') as f:
    lines = f.readlines()
for line in lines:
    line = line.strip()
    line = line.split(' ')
...
x_test = []
with io.open('/u/training/tra335/HW5/1a/preprocessed_data/imdb_test.txt','r',encoding='utf-8') as f:
    lines = f.readlines()
for line in lines:
    line = line.strip()
    line = line.split(' ')
...
for epoch in range(0,75):
    ...
    print('Train','epoch', epoch,'Acc',"%.2f" % (epoch_acc*100.0),'loss',"%.4f" % epoch_loss,'time',"%.4f" % float(time.time()-time1))
    ...
    if((epoch+1)%1==0):
        ...
        print("test",'Acc',"%.2f" % (epoch_acc*100.0),'loss',"%.4f" % epoch_loss,'time',"%.4f" % float(time.time()-time1))
...
torch.save(model,'language.model')(y_train)
